...
import pdb
from shape_aware_loss import shape_smoothness_loss
from local_context_loss import local_dice_loss
from csc_transforms import style_1, style_2
...
parser.add_argument('--root_path', type=str, default='/home/UserData/BCP_PROJECT/data/data', help='Name of Dataset')
...
parser.add_argument('--lambda_lcl', type=float, default=0.1)
parser.add_argument('--lambda_shape', type=float, default=1e-3)
# -- setting of BCP
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # --- cross-style intensity tranforms for unlabeled images ---
            unimg_a_T = style_1(unimg_a) # teacher style (S1)
            unimg_b_T = style_1(unimg_b) 
            unimg_a_S = style_2(unimg_a) # student style (S2)
            unimg_b_S = style_2(unimg_b)

            with torch.no_grad():
                unoutput_a, _ = ema_model(unimg_a_T)
                unoutput_b, _ = ema_model(unimg_b_T)
                plab_a = get_cut_mask(unoutput_a, nms=1)
                plab_b = get_cut_mask(unoutput_b, nms=1)
                img_mask, loss_mask = context_mask(img_a, args.mask_ratio)
            consistency_weight = get_current_consistency_weight(iter_num // 150)

            mixl_img = img_a * img_mask + unimg_a_S * (1 - img_mask)
            mixu_img = unimg_b_S * img_mask + img_b * (1 - img_mask)
            ...
            # --- Local Context Loss (Phase 2A) ---
            lcl_loss = local_dice_loss(outputs_l, mixl_lab, grid=(2, 2, 2))
            # --------------------------------------

            # --- Shape-aware smoothness (Phase 2B) ---
            shape_loss_l = shape_smoothness_loss(outputs_l)
            shape_loss_u = shape_smoothness_loss(outputs_u)
            shape_loss = shape_loss_l + shape_loss_u
            # -----------------------------------------

            loss = loss_l + loss_u + args.lambda_lcl * lcl_loss + args.lambda_shape * shape_loss
