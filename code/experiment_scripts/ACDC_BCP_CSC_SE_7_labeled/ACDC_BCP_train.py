...
from networks.net_factory import BCP_net, BCP_net_se, net_factory
...
def pre_train(args, snapshot_path):
    ...
    if args.model == "unet_se":
        model = BCP_net_se(in_chns=1, class_num=num_classes)
    else:
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    if args.model == "unet_se":
        model = BCP_net_se(in_chns=1, class_num=num_classes)
        ema_model = BCP_net_se(in_chns=1, class_num=num_classes, ema=True)
    else:
        model = BCP_net(in_chns=1, class_num=num_classes)
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            uimg_a, uimg_b = volume_batch[args.labeled_bs:args.labeled_bs + unlabeled_sub_bs], volume_batch[args.labeled_bs + unlabeled_sub_bs:]
                        # --- Cross-style intensity transforms for unlabeled images (CSC) ---
            uimg_a_T = style_1(uimg_a)  # teacher style (S1)
            uimg_b_T = style_1(uimg_b)
            uimg_a_S = style_2(uimg_a)  # student style (S2)
            uimg_b_S = style_2(uimg_b)
            # -------------------------------------------------------------------
            ulab_a, ulab_b = label_batch[args.labeled_bs:args.labeled_bs + unlabeled_sub_bs], label_batch[args.labeled_bs + unlabeled_sub_bs:]
            lab_a, lab_b = label_batch[:labeled_sub_bs], label_batch[labeled_sub_bs:args.labeled_bs]
            with torch.no_grad():
                pre_a = ema_model(uimg_a_T)
                pre_b = ema_model(uimg_b_T)
            ...
            net_input_unl = uimg_a_S * img_mask + img_a * (1 - img_mask)
            net_input_l = img_b * img_mask + uimg_b_S * (1 - img_mask)
